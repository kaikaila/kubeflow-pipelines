...
func (s *RunStore) UpdateRun(run *model.Run) error {
	...
	// PluginsOutput is only updated when explicitly set by the caller.
	// A nil pointer means "leave unchanged" so normal state-update callers
	// don't accidentally overwrite it.
	// Note: PluginsInput is intentionally omitted — it is immutable after run creation.
